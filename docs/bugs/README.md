...
| `0030-write-deadline-positive-control-is-calibrated-to-one-machine.md` | `write_deadline`'s positive control drove a fixed 32-thread load, one or two above the ~30-in-flight threshold a 1ms deadline needs at 33µs/write — armed on an 8-core box, never armed on a 2-vCPU CI runner, so it held main red with "nothing was refused", which reads as an unwired knob rather than a condition never created (FIXED) |
