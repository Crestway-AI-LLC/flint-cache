...
| [0031](0031-invalidating-a-near-cache-entry-the-proxy-did-not-write.md) | Invalidating a near-cache entry this proxy did not write: v1's contract IS the TTL, the exposure is cross-client, and a naive cross-proxy invalidation is WORSE than v1 for tenants who also read replicas, because a refill from a lagging replica re-caches the stale value with a fresh TTL (proposed) |
