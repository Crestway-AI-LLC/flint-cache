...
| [0029](0029-separate-the-read-lane-from-the-write-lane.md) | Separate the read lane from the write lane at the proxy's connection key: a backend connection is a strict FIFO because "position is the whole correspondence" (ADR-0021), so a slow write blocks every read queued behind it -- and ADR-0026 measured what a slow write is, an L0 stall that pins all 60 connections in flight with the master 92% idle. ADR-0021 removed head-of-line blocking behind STRANGERS' requests and could not touch blocking behind your own. Lane joins the connection key rather than opening a second pool, and read-your-own-writes is held by a barrier -- a client with a write in flight reads on the WRITE lane until it acks -- because splitting naively lets a GET overtake the SET in front of it. **ACCEPTED 2026-09-06** (proposed the same day) on its gating measurement, which was made rather than assumed. It was gated on one number: no drill measures read latency while writes stall, so the harm is certain in KIND and unmeasured in MAGNITUDE. The withdrawal criterion was "read p99 within noise of baseline" -- and it IS within noise, so that criterion would have withdrawn a confirmed defect: `read_under_stall_drill.sh` on the gate box reports read p99 0.239ms, flat, while ONE READ IN 230 WAITED 555ms, with the tail three orders of magnitude above the quiet maximum on both platforms. p99 is not where this harm lives; the criterion was wrong rather than the answer, and it is kept in the file with that noted. Same trap as the tenant-isolation exit clause reworded the same day, from the opposite direction. AMENDED the same day with two qualifications found by asking how it would be OBSERVED rather than by re-reading it: the harm needs the reader and writer on the SAME worker, since connections are round-robin and pinned for life, so a two-client drill on an 8-worker proxy would show nothing and retire the ADR for the wrong reason -- the measurement runs `--workers 1` and asserts `pool_lanes` is 1; and replica reads (D7, opt-in per tenant) ALREADY separate the lanes, since writes stay on the master, which narrows this to tenants not using them and adds a partial alternative the first version failed to list -- rejected as THE answer rather than AN answer, because it makes consistency the price of read isolation |
| [0030](0030-fleet-growth-and-a-tenants-proxy-subset.md) | Fleet growth and a tenant's proxy subset: membership changes reach subsets in one direction only, and it is the one that degrades -- ACCEPTED and implemented 2026-09-14, DelProxy now refills the hole it makes to the subset's own width |
| [0031](0031-invalidating-a-near-cache-entry-the-proxy-did-not-write.md) | Invalidating a near-cache entry this proxy did not write: v1's contract IS the TTL, the exposure is cross-client, and a naive cross-proxy invalidation is WORSE than v1 for tenants who also read replicas, because a refill from a lagging replica re-caches the stale value with a fresh TTL -- DECIDED 2026-09-14: ceiling 60s -> 30s, 5s default kept, invalidation deferred because a proxy has ONE listener and only admin digests, so the cost is a new internal surface rather than the invalidation |
| [0032](0032-one-implementation-of-every-control-plane-mutation.md) | One implementation of every control-plane mutation: `--raft` never reaches main.rs's dispatch, so every mutating verb exists twice over two state types with two durable formats -- a hand-written one and a serde one -- plus a third copy of the lease rows in the single-node fast mirror. **Four defects in eight days**, including ADR-0030's refill landing in one path with six green unit tests and no product change, and BUG-0150, where BUG-0065's fix AND the structural test holding it shut both lived in main.rs while the test's own forbidden literal sat in registry.rs. A verb table catches the arm somebody forgot to ADD and nothing textual catches the arm somebody forgot to UPDATE, which is the expensive half -- **ACCEPTED 2026-09-15 (Jeff): candidate A**, single-node runs the same state machine, staged so the durable-format change moves on its own. The ADR's premise that single-node was a small-deployment topology was wrong and is corrected in it: production is three nodes minimum, so the un-unified path is the one every drill runs and no fleet does, which strengthens the case rather than weakening it |
| [0048](0048-cross-slot-mget-through-the-proxy.md) | Cross-slot MGET through the proxy: Flint presents as one standalone server, so frameworks never colocate keys, and Rails' `read_multi` is refused and silently returns nothing -- split MGET per slot in the proxy, keep MSET atomic and refused, leave cross-slot transactions out -- ACCEPTED 2026-09-25 (option B) and built: `split_mget` |
| [0049](0049-cross-slot-rename-and-spring-sessions-login.md) | Cross-slot RENAME and Spring Session's login: Spring Security's `changeSessionId` renames the session key across slots and is refused, so a login fails -- ACCEPTED 2026-09-25 (option A): RENAME stays same-slot, because its contract is atomicity; a hash-tagged namespace or `migrateSession`, both measured, make login work |
| [0050](0050-what-the-excluded-commands-cost-measured.md) | What the excluded commands cost, measured: redis-py and django-redis locks never release and django-redis `incr` is not atomic (Lua), Flask-Caching's and Spring Cache's `clear()` fail (`KEYS`), framework batch calls hit cross-slot transactions and `MSET` -- recognise the few measured Lua scripts natively, answer `KEYS` at the proxy from `SCAN` with a cap, keep transactions and `MSET` atomic -- ACCEPTED 2026-09-26 and built; amended the same day to recognise node `redlock`'s, `redsync`'s and Ruby `redlock`'s scripts, with `python-redis-lock` recorded as unsupportable, and node `rate-limit-redis` 6.x's |
| [0051](0051-rate-limiters-measured-and-running-lua.md) | Rate limiters on Flint, measured: Python `limits` (Flask-Limiter, SlowAPI), Go `redis_rate` and node `rate-limiter-flexible` fail, because their Lua scripts write more than once or compute on the server clock in floating point; Rack::Attack works and `rate-limit-redis` 6.x is recognised under ADR-0050 -- recommends running single-slot Lua in an embedded, sandboxed Lua 5.1, each script committed as one batch as a transaction is -- PROPOSED 2026-09-26 |
