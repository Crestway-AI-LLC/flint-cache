...
# Each seat starts inside its own subshell so THIS shell never owns it as a
# job. A shell that owns a background job announces how it died, so the
# SIGKILL in cleanup prints "Killed: 9" after the PASS line and a clean run
# reads as though something went wrong. Same reason gates.sh's conformance
# stage spawns this way. Nothing here needs $!: RPID is found by pattern
# below, after the seat is listening.
...
# END-ANCHORED. A bare "--port 6456" is a substring match, so it would also
# select a seat on 64560 — the same truncation family that had a pkill on
# "--port 67" reaching three of node_tls's ports. The port is the last thing
# on this seat's argv, so requiring a space or end-of-line after it is exact.
RPID=$(pgrep -f "flint-server --port $RPORT( |\$)" | head -1)
[ -n "$RPID" ] || { echo "FAIL: no replica seat found on :$RPORT to stop"; exit 1; }
