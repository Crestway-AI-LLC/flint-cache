...
/// N RESP requests out in ONE write, then N replies read back in order.
///
/// This is the whole point of the pipelining fix. `call_raw` sends one frame
/// and BLOCKS for its reply, and the connection loop calls it once per
/// decoded command — so a client that pipelines 16 GETs used to produce 16
/// sequential proxy→node round trips, paying the internal hop per command
/// instead of once per batch. Measured 2026-08-16: pipelining bought Flint
/// 1.1-1.6x while ElastiCache got 4.6x from it, and the pipelined gap was
/// 11.4x against 2.55x unpipelined.
///
/// Ordering is the correctness argument and it needs no correlation ids: RESP
/// replies arrive on a connection in request order, so reply[i] belongs to
/// frames[i]. That is only true while nothing else shares this socket, which
/// is why backends are pinned per client connection.
///
/// Returns only once every reply has been decoded, so a short read cannot
/// leave the socket holding replies that the next exchange would mistake for
/// its own.
fn call_raw_batch(
    stream: &mut flint_tls::Stream,
    buf: &mut Vec<u8>,
    ...
    let mut wire = Vec::with_capacity(frames.iter().map(|f| f.len()).sum());
    for f in frames {
...
impl Backends {
    ...
    /// Keyed traffic, N frames at a time: one write, N replies in order.
    ///
    /// Same connection pool and same poisoning rule as `call` — and the rule
    /// matters more here. A failure mid-batch can leave unread replies in the
    /// socket, so reusing it would hand the NEXT exchange somebody else's
    /// answer. Dropping the connection is what makes that unrepresentable.
    fn call_batch(&mut self, addr: &str, frames: &[&[u8]]) -> std::io::Result<Vec<Value>> {
        self.ensure_conn(addr)?;
        let Some((stream, buf)) = self.conns.get_mut(addr) else {
            return Err(std::io::Error::other("conn cache"));
        };
        let result = call_raw_batch(stream, buf, frames);
        if result.is_err() {
            self.conns.remove(addr);
        }
        result
...
/// Longest run of client-pipelined reads the proxy will gather into one
/// backend exchange. Bounds both the memory held per connection and the
/// latency the FIRST reply in a batch waits for; past this the marginal
/// saving is small because the internal hop is already amortised.
const MAX_READ_BATCH: usize = 64;

/// Can this command ride in a pipelined read batch?
///
/// Deliberately narrow, and reads only. A read is idempotent, so when a batch
/// meets anything unexpected its members can simply be re-run on the ordinary
/// single-command path and land exactly where the unbatched proxy would have.
/// A batched WRITE has no such escape: a connection that breaks mid-batch
/// leaves N commands of unknown outcome instead of one, which widens the
/// ambiguity window that failover recovery depends on. Writes keep the old
/// path until that is worth designing for.
///
/// Every exclusion below names a path this batch cannot reproduce — answering
/// locally, fanning out across masters, carrying per-node cursor state,
/// mutating connection state, or rewriting a reply for the client's dialect.
fn batchable_read(topo: &Topology, args: &[Vec<u8>], txn: &ProxyTxn, local_cache: bool) -> bool {
    let Some(name) = args.first() else {
        return false;
    };
    if !flint_commands::is_read_command(name) {
        return false;
    }
    let upper = name.to_ascii_uppercase();
    // Answered by the proxy itself, fanned out over every master, or holding
    // a cursor that lives on the node that minted it.
    if upper.starts_with(b"FLINT")
        || matches!(
            upper.as_slice(),
            b"PING" | b"ECHO" | b"QUIT" | b"SCAN" | b"DBSIZE" | b"FLUSHALL"
        )
    {
        return false;
    }
    if is_internal_only(name) || topo.family_registered(name) {
        return false;
    }
    // The RESP3 renest / kind-differs rewrites live in `forward`. Excluding
    // those commands keeps ONE implementation of the quirk rather than a
    // second one here that would drift.
    if flint_resp::resp3_nests_reply(name) || flint_resp::resp3_differs_in_kind(name) {
        return false;
    }
    // A transaction queues on one backend CONNECTION; the near-cache can
    // answer with no backend at all. Both are single-path business.
    if txn.open || txn.addr.is_some() || local_cache {
        return false;
    }
    true
}

/// Where a batchable read must go, or `None` if it is not cleanly routable.
/// A key is required: keyless commands land on pair 0 by a different rule and
/// are left to the single path rather than duplicating it.
fn batch_target(
    topo: &Topology,
    ns: &[u8],
    args: &[Vec<u8>],
    ...
    if read_replica {
        topo.route_replica(ns, slot)
            .or_else(|| topo.route(ns, slot))
    } else {
        topo.route(ns, slot)
    }
}

/// True for the backend errors that mean "this is no longer the right node,
/// go again" — the class `forward` absorbs by rerouting. A batch cannot
/// reroute mid-flight, so any of these sends its command back to the single
/// path where that machinery already lives.
fn needs_reroute(v: &Value) -> bool {
    matches!(v, Value::Error(e)
        if e.starts_with("MOVED ") || e.starts_with("TRYAGAIN") || e.starts_with("READONLY"))
}

/// The pipelining fast path: when a client has already sent several plain
/// reads bound for the same backend, answer them with ONE round trip instead
/// of one apiece. Returns the number of BYTES it consumed from `buf`, or
/// `None` when there was no run worth batching — in which case nothing has
/// been consumed, charged or sent, and the caller proceeds exactly as before.
///
/// Gathering is deliberately side-effect free, so bailing out is always safe.
/// Only once a batch is committed does anything charge a meter or reach a
/// socket.
#[allow(clippy::too_many_arguments)]
fn try_pipelined_reads(
    topo: &Topology,
    ...
    local_cache: bool,
    async_writes: bool,
    proto: flint_resp::Proto,
    out: &mut Vec<u8>,
    hotkey_tick: &mut u32,
) -> Option<usize> {
    // ---- gather: decode ahead, commit nothing ----------------------------
    let mut cmds: Vec<(Vec<Vec<u8>>, Vec<u8>)> = Vec::new();
    let mut scanned = 0usize;
    let mut target: Option<String> = None;
    while cmds.len() < MAX_READ_BATCH {
        let rest = &buf[scanned..];
        // Inline commands (`GET k\r\n`) are rewritten by the connection loop.
        // Batching only proper RESP arrays leaves that path untouched.
        if rest.first() != Some(&b'*') {
            break;
        }
        let Ok(Decoded::Complete(frame, used)) = decode(rest) else {
            break;
        };
        let Some(args) = frame_to_args(frame) else {
            break;
        };
        if !batchable_read(topo, &args, txn, local_cache) {
            break;
        }
        let Some(addr) = batch_target(topo, ns, &args, replica_reads) else {
            break;
        };
        // A run ends at the first command bound elsewhere; the next call
        // picks that one up as the head of its own run.
        if target.as_ref().is_some_and(|t| *t != addr) {
            break;
        }
        target = Some(addr);
        cmds.push((args, rest[..used].to_vec()));
        scanned += used;
    }
    if cmds.len() < 2 {
        return None;
    ...
    // ---- commit: from here every command is accounted for ----------------
    // The quota gate must run BEFORE anything reaches a backend — a shed
    // command must not touch one. Charging here rather than during gather is
    // what makes an abandoned gather free.
    let mut sendable: Vec<(Vec<Vec<u8>>, Vec<u8>)> = Vec::with_capacity(cmds.len());
    let mut shed: Option<Value> = None;
    let mut consumed = 0usize;
    for (args, raw) in cmds {
        if let Some(name) = args.first()
            && let Some(v) = topo.quota_gate(ns, name, false, false)
        {
            // The run stops at the shed command: it is answered, and anything
            // after it stays in the buffer for the next pass.
            consumed += raw.len();
            shed = Some(v);
            break;
        }
        ...
        *hotkey_tick = hotkey_tick.wrapping_add(1);
        ...
        {
            topo.hotkeys.observe(ns, key, false);
        }
        consumed += raw.len();
        sendable.push((args, raw));
    }
    ...
    // Tenant-perceived latency for the batch. Attributed WHOLE to each member
    // rather than divided: the client cannot read any reply until the batch is
    // written, so every command really did wait this long. Overstating our own
    // latency is the safe direction for a self-measurement.
    let started = Instant::now();
    let frames: Vec<&[u8]> = sendable.iter().map(|(_, raw)| raw.as_slice()).collect();
    let batched = if frames.is_empty() {
        Vec::new()
    } else {
        b.call_batch(&addr, &frames).unwrap_or_default()
    };

    // A short or failed batch, and any reply meaning "go to a different node",
    // fall back to the single-command path. `handle` — not `data_command` —
    // because the quota was already charged above, and `handle` is where the
    // MOVED/failover retry machinery already lives.
    let mut replies: Vec<Value> = Vec::with_capacity(sendable.len());
    for (i, (args, raw)) in sendable.iter().enumerate() {
        let reply = match batched.get(i) {
            Some(v) if !needs_reroute(v) => v.clone(),
            _ => handle(topo, b, ns, args, raw, replica_reads),
        };
        replies.push(reply);
    }
    ...
    for reply in &replies {
        topo.latency.observe(ns, false, elapsed);
        if let Value::Error(msg) = reply {
            topo.errors.observe(Some(ns), errors::classify(msg));
        }
        encode_proto(reply, proto, out);
    }
    if let Some(v) = shed {
...
fn serve_client<S: Read + Write>(mut stream: S, topo: Arc<Topology>) -> std::io::Result<()> {
    ...
    loop {
        ...
        loop {
            ...
            // Pipelining fast path (see try_pipelined_reads): a run of plain
            // reads bound for one backend goes out as a single exchange
            // instead of one round trip each. Returns None — having done
            // nothing at all — whenever the next commands are not such a run,
            // so the ordinary path below stays the default.
            //
            // Channels are excluded: their per-command deadline and budget are
            // connection state the batch would have to re-implement, and a
            // co-processor session is not the pipelined-client case anyway.
            if channel_deadline.is_none()
                && let Some(ns) = authed_ns.as_deref()
                && let Some(used) = try_pipelined_reads(
                    &topo,
                    &mut backends,
                    &txn,
                    ns,
...
mod route_tests {
    ...
    /// The batch fast path is only safe because of what it REFUSES. Each
    /// exclusion below is a command whose ordinary path does something a
    /// single-backend batch cannot reproduce — answer locally, fan out across
    /// masters, carry per-node cursor state, or mutate the connection. If any
    /// of these ever starts returning true, the batch silently changes that
    /// command's semantics rather than failing, which is why they are asserted
    /// one by one instead of trusting the predicate's shape.
    #[test]
    fn only_plain_routable_reads_are_batchable() {
        let t = two_pair_topo();
        let idle = ProxyTxn::default();

        assert!(
            batchable_read(&t, &cmd(&["GET", "k"]), &idle, false),
            "a plain GET is the whole point of the fast path"
        );

        for bad in [
            vec!["SET", "k", "v"], // a write: no idempotent re-run on failure
            vec!["DEL", "k"],      // ditto
            vec!["SCAN", "0"],     // cursor state lives on the node
            vec!["DBSIZE"],        // fans out over every master
            vec!["FLUSHALL"],      // fans out, and is a write
            vec!["PING"],          // answered by the proxy itself
            vec!["ECHO", "x"],     // ditto
            vec!["FLINTINFO"],     // admin surface, refused at the edge
        ] {
            assert!(
                !batchable_read(&t, &cmd(&bad), &idle, false),
                "{bad:?} must not be batched"
            );
        }

        // The near-cache can answer with no backend at all, so a batch would
        // bypass it and quietly drop the tenant's opted-in staleness contract.
        assert!(
            !batchable_read(&t, &cmd(&["GET", "k"]), &idle, true),
            "near-cache reads stay on the single path"
        );

        // A transaction queues on ONE backend connection. Batching a read
        // inside it would send the command outside that queue, so EXEC would
        // silently skip it.
        let open = ProxyTxn {
            open: true,
            addr: None,
        ...
        assert!(
            !batchable_read(&t, &cmd(&["GET", "k"]), &open, false),
            "an open MULTI stays on the single path"
        );
        let bound = ProxyTxn {
            open: false,
            addr: Some("a:1".into()),
        ...
        assert!(
            !batchable_read(&t, &cmd(&["GET", "k"]), &bound, false),
            "a transaction bound by WATCH stays on the single path"
        );
    }

    /// A run must not span backends: replies come back in request order on ONE
    /// connection, so a batch mixing two targets would reassemble wrongly.
    #[test]
    fn batch_target_follows_the_key_and_needs_one() {
        let t = two_pair_topo();
        let a = batch_target(&t, b"0", &cmd(&["GET", "k1"]), false);
        let b = batch_target(&t, b"0", &cmd(&["GET", "k2"]), false);
        assert!(a.is_some() && b.is_some(), "keyed reads must route");
        // Keyless reads land on pair 0 by a different rule; left to the
        // single path rather than duplicating that rule here.
        assert!(
            batch_target(&t, b"0", &cmd(&["DBSIZE"]), false).is_none(),
            "a keyless command has no batch target"
        );
    }

    #[test]
    fn reroute_errors_are_recognised() {
        assert!(needs_reroute(&Value::Error("MOVED 1234 a:1".into())));
        assert!(needs_reroute(&Value::Error("TRYAGAIN later".into())));
        assert!(needs_reroute(&Value::Error("READONLY replica".into())));
        // An ordinary command error is the ANSWER, not a routing signal —
        // sending it back through the single path would re-run the command
        // and charge the tenant twice for one mistake.
        assert!(!needs_reroute(&Value::Error("ERR wrong type".into())));
        assert!(!needs_reroute(&Value::Bulk(Some(b"v".to_vec()))));
    }
...
mod batch_tests {
    ...
    /// A backend that answers NOTHING until it holds all `n` commands.
    ///
    /// This is the positive control for the pipelining fix, and it is built to
    /// fail loudly rather than quietly: if the proxy ever goes back to
    /// one-frame-then-block, this listener sits waiting for command 2 while
    /// the proxy sits waiting for reply 1, and the client's read timeout ends
    /// the test with a message that says so. A test that merely checked "N
    /// replies came back in order" would pass just as happily against the
    /// serialized implementation this fix exists to remove.
    fn withholding_backend(n: usize) -> std::net::SocketAddr {
        ...
        std::thread::spawn(move || {
            ...
            loop {
                let mut complete = 0;
                let mut off = 0;
                while let Ok(Decoded::Complete(_, used)) = decode(&buf[off..]) {
                    off += used;
                    complete += 1;
                }
                if complete >= n {
            ...
            // Only now does anything go back the other way.
            let mut out = Vec::new();
            for i in 0..n {
                encode(&Value::Bulk(Some(format!("v{i}").into_bytes())), &mut out);
            }
    ...
    #[test]
    fn a_read_batch_is_sent_before_any_reply_is_awaited() {
        const N: usize = 8;
        let addr = withholding_backend(N);
        let sock = TcpStream::connect(addr).expect("connect");
        sock.set_read_timeout(Some(Duration::from_secs(5)))
            .expect("read timeout");
        let mut stream = flint_tls::Stream::Plain(sock);
        let mut buf = Vec::new();
        let frames: Vec<Vec<u8>> = (0..N)
            .map(|i| encode_cmd(&[b"GET", format!("k{i}").as_bytes()]))
            .collect();
        let refs: Vec<&[u8]> = frames.iter().map(|f| f.as_slice()).collect();

        let replies = call_raw_batch(&mut stream, &mut buf, &refs).expect(
            "batch never completed — the backend was still waiting for later commands, \
             which means the proxy sent one frame and blocked for its reply",
        );

        assert_eq!(replies.len(), N, "one reply per command");
        for (i, r) in replies.iter().enumerate() {
            assert_eq!(
                r,
                &Value::Bulk(Some(format!("v{i}").into_bytes())),
                "reply {i} is out of order — batching relies on RESP replies \
                 arriving in request order"
            );
        }
    }

    /// Answers every command it decodes, numbering replies in arrival order,
    /// so a caller can tell which exchange a reply belongs to.
    fn counting_backend() -> std::net::SocketAddr {
        let listener = TcpListener::bind("127.0.0.1:0").expect("bind");
        let addr = listener.local_addr().expect("addr");
        std::thread::spawn(move || {
            let Ok((mut sock, _)) = listener.accept() else {
            ...
            loop {
                ...
                while let Ok(Decoded::Complete(_, used)) = decode(&buf) {
                    buf.drain(..used);
                    encode(&Value::Bulk(Some(format!("v{n}").into_bytes())), &mut out);
                    n += 1;
                }
    ...
    #[test]
    fn a_batch_leaves_no_unread_replies_behind() {
        // A batch that returned early would strand replies in the socket, and
        // the NEXT exchange on that connection would read somebody else's
        // answer. Two batches back to back on one connection is what catches
        // it: the second one's first reply would be the first one's leftover.
        const N: usize = 4;
        let addr = counting_backend();
        let sock = TcpStream::connect(addr).expect("connect");
        sock.set_read_timeout(Some(Duration::from_secs(5)))
            .expect("read timeout");
        let mut stream = flint_tls::Stream::Plain(sock);
        ...
        let first = call_raw_batch(&mut stream, &mut buf, &refs).expect("first batch");
        let second = call_raw_batch(&mut stream, &mut buf, &refs).expect("second batch");

        assert_eq!(first.len(), N);
        assert_eq!(second.len(), N);
        assert_eq!(
            second[0],
            Value::Bulk(Some(b"v4".to_vec())),
            "second batch read a leftover from the first"
        );
    }
}
