...
/// The hot loop: round-robin SETs over the hot keys, forever, through kills.
/// Every attempt is recorded before it is sent (written = the time-travel
/// ceiling); only an OK ack advances the durability floor. An ack arriving
/// after a gap longer than BLACKOUT_MIN records the gap. Pauses requested by
/// the auditor park the loop and are excluded from blackout accounting.
fn writer_loop(sh: Arc<Shared>, proxy_port: u16, key_count: usize, t0: Instant) {
...
fn main() {
    ...
    println!(
        "hotkey-chaos: {kills} master kills over {key_count} hot keys, writer runs THROUGH \
         the kills, blackout budget {blackout_budget_ms} ms (published RTO), \
         path=client->proxy->node, driver=controller"
    );
    ...
    let writer = {
        let sh = Arc::clone(&sh);
        std::thread::spawn(move || writer_loop(sh, proxy_port, key_count, t0))
    };
    ...
    for kill in 1..=kills {
        ...
        while !sh.parked.load(Ordering::SeqCst) {
        ...
        // Park the writer and audit the hot keys through the new master.
        sh.pause.store(true, Ordering::SeqCst);
        while !sh.parked.load(Ordering::SeqCst) {
            std::thread::sleep(Duration::from_millis(5));
        }
    ...
    // Let the writer settle, then stop it and run the final audit.
    std::thread::sleep(Duration::from_millis(spell_ms));
